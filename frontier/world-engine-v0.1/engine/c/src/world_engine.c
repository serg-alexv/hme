...
static const int32_t BIND_Q15[WORLD_JOINT_COUNT][WORLD_WAVE_COUNT] = {
...
static const int32_t JOINT_LIMIT_MDEG[WORLD_JOINT_COUNT] = {
    8000, 18000, 18000, 22000, 36000, 36000, 32000, 36000, 40000
};

enum {
    WORLD_Q16_ATTUNE_ENTER = 16384,
    WORLD_Q16_ATTUNE_EXIT = 10486,
    WORLD_Q16_STORM_VARIABILITY_ENTER = 36044,
    WORLD_Q16_STORM_NOVELTY_ENTER = 39321,
    WORLD_Q16_STORM_VARIABILITY_EXIT = 24903,
    WORLD_Q16_STORM_NOVELTY_EXIT = 26214
};
...
static world_behavior_t candidate_for(world_behavior_t current, const world_config_t *config, const world_input_frame_t *input) {
    if (!input->valid || input->freshness_ms > config->stale_after_ms) return WORLD_BEHAVIOR_STALE;
    ...
    if (current == WORLD_BEHAVIOR_STALE) return WORLD_BEHAVIOR_CALM;
    if (current == WORLD_BEHAVIOR_STORM) return storm_hold ? WORLD_BEHAVIOR_STORM : WORLD_BEHAVIOR_ATTUNE;
    if (current == WORLD_BEHAVIOR_ATTUNE) {
        if (storm_enter) return WORLD_BEHAVIOR_STORM;
        return calm_region ? WORLD_BEHAVIOR_CALM : WORLD_BEHAVIOR_ATTUNE;
    }
    if (storm_enter) return WORLD_BEHAVIOR_STORM;
    return attune_enter ? WORLD_BEHAVIOR_ATTUNE : WORLD_BEHAVIOR_CALM;
}

static uint16_t dwell_for(world_behavior_t current, world_behavior_t candidate, const world_config_t *config) {
    if (candidate == WORLD_BEHAVIOR_STALE) return 1;
    if (candidate == WORLD_BEHAVIOR_STORM) return config->dwell_storm_ticks;
    if (candidate == WORLD_BEHAVIOR_ATTUNE) return config->dwell_attune_ticks;
    if (current == WORLD_BEHAVIOR_STALE) return config->dwell_recover_ticks;
...
void world_default_config(world_config_t *config) {
    ...
    config->attune_enter_q16 = WORLD_Q16_ATTUNE_ENTER;
    config->attune_exit_q16 = WORLD_Q16_ATTUNE_EXIT;
    config->storm_variability_enter_q16 = WORLD_Q16_STORM_VARIABILITY_ENTER;
    config->storm_novelty_enter_q16 = WORLD_Q16_STORM_NOVELTY_ENTER;
    config->storm_variability_exit_q16 = WORLD_Q16_STORM_VARIABILITY_EXIT;
    config->storm_novelty_exit_q16 = WORLD_Q16_STORM_NOVELTY_EXIT;
...
void world_engine_init(world_engine_t *engine) {
    ...
    engine->behavior = WORLD_BEHAVIOR_CALM;
    engine->candidate = WORLD_BEHAVIOR_CALM;
    for (size_t i = 0; i < WORLD_WAVE_COUNT; ++i) engine->phase_q32[i] = INITIAL_PHASE[i];
}

const char *world_behavior_name(world_behavior_t behavior) {
    switch (behavior) {
        case WORLD_BEHAVIOR_CALM: return "CALM";
        case WORLD_BEHAVIOR_ATTUNE: return "ATTUNE";
        case WORLD_BEHAVIOR_STORM: return "STORM";
        case WORLD_BEHAVIOR_STALE: return "STALE";
...
const char *world_source_name(world_source_t source) {
    switch (source) {
        case WORLD_SOURCE_SIMULATED: return "SIMULATED";
        case WORLD_SOURCE_RECORDED_REPLAY: return "RECORDED_REPLAY";
        case WORLD_SOURCE_LIVE_WLAN: return "LIVE_WLAN";
        case WORLD_SOURCE_STALE_WLAN: return "STALE_WLAN";
        default: return "UNKNOWN";
    }
}
...
uint64_t world_pose_hash(const world_pose_frame_t *pose) {
    ...
    for (size_t i = 0; i < WORLD_JOINT_COUNT; ++i) hash = fnv1a64_u32(hash, (uint32_t)pose->joint_mdeg[i]);
    return hash;
}

void world_engine_step(world_engine_t *engine, const world_config_t *config, const world_input_frame_t *input, world_pose_frame_t *output) {
    if (!engine || !config || !input || !output) return;

    world_behavior_t current = (world_behavior_t)engine->behavior;
    world_behavior_t candidate = candidate_for(current, config, input);
    ...
    if (engine->candidate_ticks >= required && candidate != current) {
        ...
        current = (world_behavior_t)engine->behavior;
    }

    int32_t wave_mdeg[WORLD_WAVE_COUNT] = {0};
    ...
    for (size_t i = 0; i < WORLD_WAVE_COUNT; ++i) {
    ...
    if (current == WORLD_BEHAVIOR_STALE) output->root_y_um -= 24000;

    for (size_t joint = 0; joint < WORLD_JOINT_COUNT; ++joint) {
        int64_t sum = 0;
        for (size_t wave = 0; wave < WORLD_WAVE_COUNT; ++wave) sum += ((int64_t)BIND_Q15[joint][wave] * wave_mdeg[wave]) >> 15;

        if (current == WORLD_BEHAVIOR_ATTUNE) {
            if (joint == J_NECK1_PITCH) sum -= 4500;
            if (joint == J_WING_L_ROLL) sum += 5000;
            if (joint == J_WING_R_ROLL) sum -= 5000;
        } else if (current == WORLD_BEHAVIOR_STALE) {
    ...
    output->frame_hash = world_pose_hash(output);
    engine->last_sequence = input->sequence;
    engine->tick++;
}

int world_pose_to_json(const world_pose_frame_t *pose, char *buffer, size_t capacity) {
    ...
    return snprintf(
        ...
        world_source_name((world_source_t)pose->source), pose->valid ? "true" : "false",
        pose->freshness_ms, world_behavior_name((world_behavior_t)pose->behavior),
