...
export class DragonNetworkStateLive extends BaseScriptComponent {
  // Written by the WorldEngine Pose Bridge. No authoritative wave generation occurs here.
  ...
  onAwake(): void {
    ...
    this.wingL = this.findFirst(["w_C_L.001", "w_C_L", "upper_arm_L", "Arm_Cont_L"]);
    this.wingR = this.findFirst(["w_C_R.001", "w_C_R", "upper_arm_R", "Arm_Cont_R"]);
    this.tail1 = this.findFirst(["tail_1", "Cont_tail.001", "Cont_tail"]);
    ...
    if (this.printDiagnostics) {
      print("[WorldEngine PoseDriver] AWAKE");
      print("[WorldEngine PoseDriver] neck1=" + this.nameOf(this.neck1));
      print("[WorldEngine PoseDriver] neck2=" + this.nameOf(this.neck2));
      print("[WorldEngine PoseDriver] head=" + this.nameOf(this.head));
      print("[WorldEngine PoseDriver] wingL=" + this.nameOf(this.wingL));
      print("[WorldEngine PoseDriver] wingR=" + this.nameOf(this.wingR));
      print("[WorldEngine PoseDriver] tail1=" + this.nameOf(this.tail1));
      print("[WorldEngine PoseDriver] tail2=" + this.nameOf(this.tail2));
      print("[WorldEngine PoseDriver] tail3=" + this.nameOf(this.tail3));
    }
  ...
  private sourceName(): string {
    ...
    if (this.sourceCode === 2) return "LIVE_NETWORK_COUNTERS";
    return "STALE_INPUT";
  }
  ...
  private applyPose(): void {
    ...
    if (this.printDiagnostics && second !== this.lastPrintedSecond) {
      ...
      print(`[WorldEngine PoseDriver] source=${this.sourceName()} state=${this.behaviorName()} seq=${this.liveSequence.toFixed(0)} fresh=${this.liveFreshnessMs.toFixed(0)}ms hash=${this.frameHashPrefix}`);
    }
  }
}
