...
export class WorldEnginePoseBridge extends CoreService {
    static descriptor() {
        return {
            id: "com.world20.world.posebridge",
            interfaces: CoreService.descriptor().interfaces,
            name: "WorldEngine Pose Bridge",
            description: "Applies deterministic world.pose/0.1 frames to the dragon.",
    ...
    start() {
        ...
        this.server.onClientConnected = () => {
            console.log("[WorldEngine Bridge] client connected");
            this.target = null;
        };
        ...
        this.server.onClientDisconnected = () => {
            console.log("[WorldEngine Bridge] client disconnected; forcing STALE");
            this.applyStale();
        };
        this.server.onClientSocketError = (error) => {
            console.error("[WorldEngine Bridge] socket error: " + error);
            this.applyStale();
        };
    ...
    receiveData(text) {
        ...
        while (true) {
            ...
            try {
                ...
                console.error("[WorldEngine Bridge] invalid JSON: " + error);
            }
        }
    }
    ...
    resolveTarget() {
        ...
        for (const component of components) {
            ...
            if (name.indexOf(TARGET_SCRIPT_NAME) >= 0) {
                ...
                console.log("[WorldEngine Bridge] target found: " + name);
                return component;
            }
        }
        console.error("[WorldEngine Bridge] DragonNetworkStateLive component not found");
        return null;
    }
    ...
    applyFrame(frame) {
        if (!frame || frame.schema !== "world.pose/0.1") {
            console.error("[WorldEngine Bridge] unsupported schema");
            return;
        }
        ...
        if (this.appliedFrames % 60 === 0) {
            console.log(`[WorldEngine Bridge] seq=${sequence} state=${frame.behavior} hash=${target.frameHashPrefix}`);
        }
    }
