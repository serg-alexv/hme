# World 2.0 Living Review

This directory contains the versioned review manuscript for the World 2.0 / LEGION program.

## Current publication

- [`world2-review-v0.1.md`](./world2-review-v0.1.md) — historical memorandum, introduction, current actual system description, and expandable full-paper contents.

## Authorship

- **Author:** Mika (`serg-alexv`), Founding CTO and Original Technical Originator.
- **Research, structuring, drafting, and editorial assistance:** OpenAI GPT-5.6 Thinking.
- **Project owner and executive principal:** Leo Shaposhnikov.

## Document class

Version 0.1 is a public working review manuscript, not a peer-reviewed article. It was published without a separate pre-publication verification round at the author's instruction. Implementation claims remain bounded by `STATUS.md`, `CLAIMS.md`, `OWNERSHIP.md`, and preserved experiment evidence.

## Revision rules

1. Major section numbers remain stable.
2. New material is added as subsections whenever possible.
3. Historical versions are preserved through Git history and tagged releases.
4. Each external factual claim should cite a primary paper, standard, official documentation, or clearly labeled secondary source.
5. Product status, funding, standards, and institutional information require a dated source.
6. World 2.0 implementation claims may not exceed the current LEGION evidence class.
7. Patent and novelty conclusions remain blocked until the dedicated prior-art layer is completed.

## Planned build order

```text
v0.1 historical memorandum + actual system + living TOC
→ v0.2 review method + telemetry foundations
→ v0.3 WLAN sensing + prior-art matrix
→ v0.4 procedural embodiment + XR architecture
→ v0.5 provenance + evaluation framework
→ v0.6 commercial/institutional landscape
→ v0.7 integrated full review
→ v1.0 publication candidate
```
